...
assert.equal(context.comparisonUnit('$7.69','3 lb (24 eggs)','').amount, 7.69 / 24, 'egg count takes priority over carton weight');
assert.equal(context.comparisonUnit('$6','18 eggs','').key, '$::egg');
assert.equal(context.comparisonUnit('$6','18 eggs','').amount, 1 / 3);
...
context.normalizeFoodName = s => String(s || '').trim().replace(/\s*\([^()]*\)\s*$/, '').toLowerCase();
vm.runInContext(fs.readFileSync('smart-shopping-store-listings.js','utf8'), context);
vm.runInContext(html.slice(html.indexOf('    function renderStoreListings('), html.indexOf('    function renderComparison(')), context);
