...
const beforeMainFacts = JSON.stringify(item.facts);
afterReload.recordStorePrice('shop::multi', {price:'$6',salePrice:'$4',weight:'2 lb',unitPrice:'$3.00 / lb',saleUnitPrice:'$2.00 / lb'}, 'Giant', '2026-09-05');
afterReload.recordStorePrice('shop::multi', {price:'$6',salePrice:'',weight:'2 lb',unitPrice:'$3.00 / lb'}, 'Walmart', '2026-09-05');
afterReload.recordStorePrice('shop::multi', {price:'$8',salePrice:'',weight:'2 lb',unitPrice:'$4.00 / lb'}, 'Giant', '2026-09-06');
const multiDevice = device();
const multi = {sourceList:'shop',key:'multi',...item};
const storesMarkup = multiDevice.storePricesMarkup(multi);
assert.ok(storesMarkup.includes('Giant') && storesMarkup.includes('Walmart') && storesMarkup.includes('Costco'));
assert.ok(storesMarkup.includes('$8') && !storesMarkup.includes('$4</td>'), 'latest Giant offer replaces its older offer only in store overview');
assert.equal(JSON.stringify(item.facts), beforeMainFacts, 'adding store offers leaves main facts intact');
...
const dateDevice = device();
const dateStale = JSON.parse(storage.get('smart-shopping-price-history-v1'));
const dateId = dateStale.item[0].id;
assert.equal(dateDevice.updatePriceHistoryDate('item', dateId, '2026-02-30'), false);
assert.equal(dateDevice.updatePriceHistoryDate('item', dateId, '2026-08-20'), true);
const dateSaved = JSON.parse(storage.get('smart-shopping-price-history-v1'));
assert.equal(dateSaved.item.length, dateStale.item.length);
assert.equal(dateSaved.item[0].price, dateStale.item[0].price);
assert.equal(dateSaved.item[0].date, '2026-08-20');
dateDevice.mergePriceHistory(dateStale);
...
oldDateDevice.localStorage.setItem = () => { throw new Error('Storage full'); };
assert.throws(() => oldDateDevice.updatePriceHistoryDate('item', dateId, '2026-08-21'));
assert.equal(oldDateDevice.activePriceHistory('item')[0].date, null);
oldDateDevice.localStorage.setItem = oldDateSet;
