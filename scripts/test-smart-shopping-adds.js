...
function device(storage = new Map(), limit = Infinity) {
  ...
  localStorage.setItem = (k,v) => {
    ...
    if ([...next.values()].reduce((total,value)=>total+value.length,0)>limit) { const error=new Error('Storage full'); error.name='QuotaExceededError'; throw error; }
    storage.set(k,v);
  };
...
(async()=>{
  ...
  assert.equal(a.storage.has('smart-shopping-item-adds-v1'),false);
  ...
  const photo='data:image/jpeg;base64,'+'x'.repeat(800);
  server.data.itemAdds['买菜'].push({key:'photo',name:'照片商品',photo});
  server.data.priceHistory={photo:[{id:'price',price:'$1.99',note:'x'.repeat(400)}]};
  const phone=device(new Map(),500); await phone.sync();
  assert.equal(phone.items()['买菜'].some(i=>i.key==='photo'),true);
  assert.equal(phone.storage.has('smart-shopping-item-adds-v1'),false);
  assert.equal(phone.storage.has('smart-shopping-price-history-v1'),false);
